...
@register_task("score_inference")
class ScoreProteinTask(UnicoreTask):
    @staticmethod
    def add_args(parser):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        is2re_dataset = pFindIs2reDataset(lmdb_dataset, self.args, is_train=is_train)
        ...
        dataset_dict["net_input"]["ion_mz"] = SeqDataset(is2re_dataset, "ion_mz")

        dataset_dict["net_input"]["index"] = ScalerDataset(is2re_dataset, "idx")
        dataset_dict["net_input"]["fdr"] = ScalerDataset(is2re_dataset, "fdr")
        dataset_dict["net_input"]["title"] = SeqStrDataset(is2re_dataset, "title")

        dataset_dict["net_input"]["denoise_pred_mask"] = SeqDataset(
            is2re_dataset, "denoise_pred_mask"
        )
        dataset_dict["net_input"]["intensity_pred_mask"] = SeqDataset(
            is2re_dataset, "intensity_pred_mask"
        )
        dataset_dict["net_input"]["rope_embeding"] = SeqDataset(
            is2re_dataset, "rope_embeding"
        )
        if self.args.train_pfind:
            dataset_dict["net_input"]["label_index"] = ScalerDataset(
                is2re_dataset, "label_index"
            )

        dataset_dict["targets"]["denoise_label"] = SeqDataset(
            is2re_dataset, "denoise_label"
        )
        dataset_dict["targets"]["intensity_label"] = SeqDataset(
            is2re_dataset, "intensity_label"
        )
        dataset_dict["targets"]["noise_peak_label"] = SeqDataset(
            is2re_dataset, "noise_peak_label"
        )
        dataset_dict["targets"]["noise_peak_mask"] = SeqDataset(
            is2re_dataset, "noise_peak_mask"
        )
        dataset_dict["targets"]["mod_label"] = SeqDataset(is2re_dataset, "mod_label")
        dataset_dict["targets"]["mod_mass_label"] = SeqDataset(
            is2re_dataset, "mod_mass_label"
        )

        dataset_dict["targets"]["modification_cls_label"] = SeqDataset(
            is2re_dataset, "modification_cls_label"
        )
        dataset_dict["targets"]["ion_res_num"] = SeqDataset(
            is2re_dataset, "ion_res_num"
        )
        dataset_dict["targets"]["ion_comp_res_num"] = SeqDataset(
            is2re_dataset, "ion_comp_res_num"
        )
        dataset_dict["targets"]["spec_label"] = SeqDataset(is2re_dataset, "spec_label")
        dataset_dict["targets"]["is_target"] = SeqDataset(is2re_dataset, "is_target")

        if self.args.res_type_pred:
            dataset_dict["targets"]["res_type_label"] = SeqDataset(
                is2re_dataset, "res_type_label"
            )
        ...
        # nce = ScalerDataset(is2re_dataset, "nce")
        # ins = ScalerDataset(is2re_dataset, "ins")
        # rt_normalized = ScalerDataset(is2re_dataset, "rt_normalized")
