...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "pUniFind_spectra_path", help="输入 pUniFind 结果 .spectra 文件路径 / Input pUniFind result .spectra file path"
    )
    args = parser.parse_args()

    # 可选：检查文件或路径是否存在 / Optional: check whether the file or path exists
    if not os.path.exists(args.pFind_path):
        parser.error("指定的 pFind 项目路径不存在 / Specified pFind project path does not exist: " + args.pFind_path)
    if not os.path.exists(args.pUniFind_spectra_path):
        parser.error(
            "指定的 pUniFind .spectra 文件不存在 / Specified pUniFind .spectra file does not exist: " + args.pUniFind_spectra_path
        )
