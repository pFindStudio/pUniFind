echo "==================================pUniFind de novo==========================================="

projects_base=./projects
project_name=$1
batch_size=$2
[ -z "${project_name}" ] && project_name=debug
# project_name=pfind3Vigna
[ -z "${is_TIMS}" ] && is_TIMS=0
...
[ -z "${max_length}" ] && max_length=40
[ -z "${min_length}" ] && min_length=6
[ -z "${num_proc}" ] && num_proc=16
...
[ -z "${pred_rt}" ] && pred_rt=0
[ -z "${denovo_pred}" ] && denovo_pred=1
[ -z "${spectrum_pred}" ] && spectrum_pred=1
...
torchrun --nproc_per_node=$n_gpu --master_port $MASTER_PORT --nnodes=$OMPI_COMM_WORLD_SIZE --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_IP \
       ./official_denovo_workflow.py --user-dir ${UNIQUE_NAME} $data_path --valid-subset $valid_subset \
       ...
       --task denovo_inference --loss cross_entropy --arch pep_ms \
       --node-dim $node_dim \
